...
/** -----------------------------
 * In-memory app state
 * (persists until the server restarts; fine for testing)
 * ------------------------------*/
const mem = {
  tenantId: '' as string,
...
// Raw body for webhook; JSON + URL-encoded for app
...
app.post('/api/settings', (req, res) => {
  ...
  s.autoApproveBills = String(req.body.autoApproveBills || s.autoApproveBills) === 'true'
  log('ok', `Settings updated: Rev ${s.revenueCode}, Cost ${s.costCode}, Accrual ${s.accrualCode}, VAT "${s.salesTaxName}", AutoApprove ${s.autoApproveBills}`)
  res.json({ ok: true })
})
...
app.post('/campaigns', async (req, res) => {
  try {
      ...
      manualJournals: [
        {
          ...
          journalLines: [
            { accountCode: mem.settings.costCode, lineAmount: Number(expectedCostNet), description: `Accrued cost ${campaignRef}` },
            { accountCode: mem.settings.accrualCode, lineAmount: -Number(expectedCostNet), description: `Accrued cost ${campaignRef}` },
          ],
        } as any,
      ],
    })
    ...
    const msg = `Created invoice ${createdInvoice?.invoiceNumber || ''} and accrued £${Number(expectedCostNet).toFixed(2)} for ${campaignRef}`
    log('ok', msg)
...
app.post('/webhooks/xero', async (req: any, res) => {
  try {
    ...
    if (!verifyXeroWebhookSignature(signature, rawBody)) return res.status(401).send('Invalid signature')
    ...
    for (const ev of json.events || []) {
      ...
      if (variance !== 0) {
        const jl =
          variance > 0
            ? [
                { accountCode: mem.settings.costCode, lineAmount: Math.abs(variance), description: `Accrual variance ${campaignRef}` },
                { accountCode: mem.settings.accrualCode, lineAmount: -Math.abs(variance), description: `Accrual variance ${campaignRef}` },
              ]
            : [
                { accountCode: mem.settings.accrualCode, lineAmount: Math.abs(variance), description: `Accrual release ${campaignRef}` },
                { accountCode: mem.settings.costCode, lineAmount: -Math.abs(variance), description: `Accrual release ${campaignRef}` },
              ]
      ...
      log('ok', `Bill ${inv.invoiceNumber || inv.invoiceID} recoded to accrual; variance £${variance.toFixed(2)} for ${campaignRef}`)
    }
...
<body>
  ...
  <script>
    async function loadSettings() {
      ...
      f.revenueCode.value = s.revenueCode;
      f.costCode.value = s.costCode;
      f.accrualCode.value = s.accrualCode;
      f.salesTaxName.value = s.salesTaxName;
      f.autoApproveBills.checked = !!s.autoApproveBills;
    }
...
/** -----------------------------
 * Webhook endpoint (HMAC validated earlier)
 * ------------------------------*/
function verifyXeroWebhookSignature(signature: string, rawBody: Buffer) {
  const key = process.env.XERO_WEBHOOK_KEY
  if (!key) return false
...
// (KEEP the /webhooks/xero route already defined above)
